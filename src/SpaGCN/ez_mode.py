...
def detect_spatial_domains_ez_mode(
    adata,
    img,
    x_array,
    y_array,
...
def spatial_domains_refinement_ez_mode(
    sample_id, pred, x_array, y_array, shape="hexagon"
):
...
def plot_spatial_domains_ez_mode(
    adata,
    domain_name,
    x_name,
    y_name,
...
def detect_SVGs_ez_mode(
    ...
    adj_2d = calculate_adj_matrix(
        x=adata.obs[x_name].tolist(), y=adata.obs[y_name].tolist(), histology=False
    )
    start, end = (
        np.quantile(adj_2d[adj_2d != 0], q=0.001),
    ...
    nbr_domians = find_neighbor_clusters(
        target_cluster=target,
        cell_id=adata.obs.index.tolist(),
        x=adata.obs[x_name].tolist(),
        y=adata.obs[y_name].tolist(),
    ...
    de_genes_info = rank_genes_groups(
        input_adata=adata,
        target_cluster=target,
...
def detect_meta_genes_ez_mode(
    adata, target, x_name, y_name, domain_name, start_gene, use_raw=False
):
